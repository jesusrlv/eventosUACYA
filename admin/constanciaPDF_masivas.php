...
require('prcd/fpdf/fpdf.php');
    ...
    $eventoSQL = "SELECT * FROM eventos WHERE id = '$id'";
    $resultadoEventoSQL = $conn->query($eventoSQL);
    $rowEvento = $resultadoEventoSQL->fetch_assoc();

    // // while($rowEventos = $resultadoEvento->fetch_assoc()){

    //     $nombre = $rowEventos['nombre'];
    //     $apellidos = $rowEventos['apellidos'];
    ...
    $apellidos = $rowEventos['apellidos'];
        ...
        $pdf->Multicell(270,12,utf8_decode('Por medio de la presente se hace constar que el(la) C.'),0,'C',0);
        $pdf->SetFont('Arial','B',12);
        $pdf->Multicell(270,10,utf8_decode(''.$nombre.' '.$apellidos.''),0,'C',0);
